...
프로젝트에 Airtable MCP 서버 및 SDK 래퍼를 설정하는 스킬. 5단계 중 필요한 단계만 선택 가능.
...
기본 설치를 진행할까요, 아니면 특정 단계만 선택할까요?
```

> **중요**: 스키마 캐싱(Step 4)은 기본 설치에 포함된다. 이 단계가 없으면 Claude가 매번 `describe_table` API를 호출해서 토큰을 낭비한다.
...
**해결책**:
- 스키마를 `.airtable/{appId}/schema.md` 파일로 저장
...
다른 프로젝트에 동일한 베이스의 스키마가 있는지 검색:

```bash
find ~/Documents/DEV -path "*/.airtable/{appId}/schema.md" -type f 2>/dev/null
```

- **{appId}**: 현재 베이스의 ID (예: `appABC123`)
- 검색 범위: `~/Documents/DEV/` 하위 전체

**검색 결과가 있으면**:
```
다음 프로젝트에서 동일한 베이스의 스키마를 찾았습니다:
- /Users/username/Documents/DEV/project-a/.airtable/appABC123/schema.md
- /Users/username/Documents/DEV/project-b/.airtable/appABC123/schema.md

이 스키마를 복제할까요? (y/n)
```

사용자 승인 시 → `.airtable/{appId}/` 디렉토리 전체 복제

**2단계: API로 자동 생성**

검색 결과가 없거나 복제를 거부한 경우:

```
기존 스키마를 찾을 수 없습니다. API로 자동 생성할까요? (y/n)
```
...
변환 규칙은 `assets/schema-generator.md`를 참조 (Task 5에서 생성됨)

**3단계: USER-ADDED 영역 안내**

생성 후 사용자에게 안내:
```
스키마가 생성되었습니다: .airtable/{appId}/schema.md
...
Airtable에서 필드가 변경된 경우, 사용자가 명시적으로 요청:
...
**주의사항**:
- ⚠️ 자동 갱신 금지 (항상 사용자 명시적 요청 시에만)
- AUTO-GENERATED 영역만 재생성 (USER-ADDED 영역 보존)
...
```
작업 전 스키마 확인 체크리스트:
1. .airtable/{appId}/schema.md 파일 존재 확인
2. 파일 읽기 (Read 도구 사용)
3. 필요한 테이블/필드 정보 확인
4. 작업 진행
```
...
.airtable/appABC123/snapshots/Users-20260128.json
.airtable/appABC123/snapshots/Payments2024-20260128.json
```
...
```
스냅샷 생성 완료:
- 파일: .airtable/appABC123/snapshots/Users-20260128.json
- 크기: 856 KB (123 레코드)
...
```bash
# Read 도구로 JSON 파일 읽기
Read .airtable/appABC123/snapshots/Users-20260128.json
...
```typescript
// JSON import
import usersSnapshot from './.airtable/appABC123/snapshots/Users-20260128.json'
